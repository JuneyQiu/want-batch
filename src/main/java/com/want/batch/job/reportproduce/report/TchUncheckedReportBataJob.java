 ...
 */
@Component
public class TchUncheckedReportBataJob {
	...
	public void buildExcelDocument(ResultSet sdActualNotchecks, Map<String, Object> directiveTblMap) throws Exception {
		...
		try {
			...
			while (sdActualNotchecks.next()) {
				...
				// 2014-06-19 mirabelle add 流水码
				this.createCell((short)b++, row, this.getStringValue(sdActualNotchecks.getString("STORE_DISPLAY_SID")), 
					cellStyle );
				...
				// 2014-06-19 mirabelle add 岗位名称
				this.createCell((short)b++, row, this.getStringValue(sdActualNotchecks.getString("JOB_NAME")), 
					cellStyle );
				
				// 2014-06-19 mirabelle add 新终端编码
				this.createCell((short)b++, row, this.getStringValue(sdActualNotchecks.getString("MDM_STORE_ID")), 
					cellStyle );
				...
				// 陈列面积;2014-06-19 mirabelle update 更改为特陈属性;当特陈形式为地堆时，显示面积；当特陈形式为包柱时，显示面数；当特陈形式有特陈属性时，显示特陈属性；否则为空白
				String displayParam = "";
				
				if ("1".equals(sdActualNotchecks.getString("DISPLAY_TYPE_SID"))) {
					
					displayParam = String.valueOf(sdActualNotchecks.getInt("DISPLAY_ACREAGE"));
				}
				else if ("2".equals(sdActualNotchecks.getString("DISPLAY_TYPE_SID"))) {
					
					displayParam = sdActualNotchecks.getString("DISPLAY_SIDECOUNT") != null ? sdActualNotchecks.getString("DISPLAY_SIDECOUNT") : "";
				}
				else if (!"1".equals(sdActualNotchecks.getString("DISPLAY_TYPE_SID")) && 
						!"2".equals(sdActualNotchecks.getInt("DISPLAY_TYPE_SID")) &&
						sdActualNotchecks.getString("DISPLAY_PARAM_ID") != null) {
					
					displayParam = sdActualNotchecks.getString("PARAMETER_NAME") != null ? sdActualNotchecks.getString("PARAMETER_NAME") : "";
				}
				
				this.createCell((short)b++, row, displayParam, cellStyle );
				
				// 2014-06-19 mirabelle add 填写时间
				this.createCell((short)b++, row, this.getStringValue(sdActualNotchecks.getDate("FIRST_INPUT_DATE")!=null ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(sdActualNotchecks.getTimestamp("FIRST_INPUT_DATE")) : ""), 
					cellStyle );
	...
	public Map<String, Object>  bulidSheetTitle(HSSFWorkbook workbook, 	
	  ...
	  // 设置默认打印为横列
		...
		// 陈列形式;2014-06-20 mirabelle update 实际特陈形式
		sheet.setColumnWidth((short) i++, (short) 3500);
		
		// 陈列面积;2014-06-20 mirabelle update 实际特陈属性
		sheet.setColumnWidth((short) i++, (short) 3500);
		...
		// 2014-06-18 mirabelle add 流水码
		this.createCell((short)a++, rowTitle3, "流水码", 
			cellStyle );
		...
		// 2014-06-18 mirabelle add 岗位名称
		this.createCell((short)a++, rowTitle3, "岗位名称", 
			cellStyle );
		
		// 2014-06-18 mirabelle add 新终端编码
		this.createCell((short)a++, rowTitle3, "新终端编码", 
			cellStyle );
		
		// 终端ID
		this.createCell((short)a++, rowTitle3, "旧终端编码", 
			cellStyle );
		...
		// 陈列形式
		this.createCell((short)a++, rowTitle3, "实际特陈形式", 
			cellStyle );
		
		// 陈列面积;2014-06-18 mirabelle update 改为实际特陈属性
		this.createCell((short)a++, rowTitle3, "实际特陈属性", 
			cellStyle );
		
		// 2014-06-18 mirabelle add 填写时间
		this.createCell((short)a++, rowTitle3, "首次填写时间", 
			cellStyle );
