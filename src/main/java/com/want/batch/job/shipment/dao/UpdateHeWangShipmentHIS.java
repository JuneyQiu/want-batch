...
@Component
public class UpdateHeWangShipmentHIS {
	private static String First_Day;
	private static String Last_Day;
	private static String Last_Yearmonth;

	protected final Log logger = LogFactory.getLog(UpdateHeWangShipmentHIS.class);
	...
	public UpdateHeWangShipmentHIS() {
		DateFormater df = new DateFormater();
		First_Day = df.getLastYearMonth(df.getCurrentYearMonth()) + "01";
		Last_Day = df.getLastDayOfMonth(
				df.getLastYearMonth(df.getCurrentYearMonth()), "yyyyMMdd");
		Last_Yearmonth = df.getLastYearMonth(df.getCurrentYearMonth());
		logger.info("FirstDay" + First_Day + "--Last_Day" + Last_Day
				+ "--Last_Yearmonth" + Last_Yearmonth);
	}
	 ...
	 * @throws SQLException 
	 */
	private String getMappingID(StockBO sbo, Connection conn) throws SQLException {
	...
	private boolean insertShipment(StockBO sbo, Connection conn) throws SQLException {

		String sql = "insert into WSIM_TC_TBL_HIS (PROJECT_SID,CUSTOMER_ID,MATERIAL_ID,QTY,UPDATE_DATE,COMPANY_KEY,YEARMONTH) values(1,?,?,?,sysdate,1,?) ";
		boolean flag = false;
		...
		pstmt.setString(4, Last_Yearmonth);
		pstmt.execute();
		flag = true;
		pstmt.close();
		return flag;
	...
	private boolean updateShipment(StockBO sbo, Connection conn) throws SQLException {
		String sql = "update WSIM_TC_TBL_HIS set QTY = ?,UPDATE_DATE=sysdate  where CUSTOMER_ID = ? and MATERIAL_ID = ? and YEARMONTH = ? ";
		boolean flag = false;
		PreparedStatement pstmt = conn.prepareStatement(sql);
	...
	private StockBO isSaved(StockBO sbo, Connection conn) throws SQLException {
		String sql = "select CUSTOMER_ID,MATERIAL_ID,QTY,UPDATE_DATE from WSIM_TC_TBL_HIS where CUSTOMER_ID=? and MATERIAL_ID=? and YEARMONTH = ?";

		PreparedStatement pstmt = conn.prepareStatement(sql);
		pstmt.setString(1, sbo.getCUSTOMER_ID());
		pstmt.setString(2, sbo.getMATERIAL_ID());
		pstmt.setString(3, Last_Yearmonth);
		ResultSet rs = pstmt.executeQuery();
		StockBO bo = null;
