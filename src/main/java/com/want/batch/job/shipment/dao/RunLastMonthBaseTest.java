...
public class RunLastMonthBaseTest {

	protected final Log logger = LogFactory.getLog(RunLastMonthBaseTest.class);

	@Autowired
	private DataSource hw09DataSource;

	public RunLastMonthBaseTest() {
	}
	...
	private void doUpdateStockHis(Connection conn_h) throws SQLException {
		...
		SQL.append(" (select TCOMPANY_SEQ_ID,PRODUCT_SEQ_ID,QUANTITY,YEARMONTH from TRANS_CUSTOMER_STOCK_MERGE where YEARMONTH='"
				+ LAST_YEARMONTH + "') a left join  ");
		SQL.append(" (select b.SID as TCOMPANY_SEQ_ID ,c.SID as PRODUCT_SEQ_ID ,a.QTY as QUANTITY  from TRANS_SAP_AFTER25SHIPMENT_TEMP a inner join FORWARDER_INFO_TBL b on upper(a.CUSTOMER_ID)=upper(b.FORWARDER_ID) inner join  PROD_INFO_TBL c  on a.MATERIAL_ID=c.PROD_ID ) b ");
		...
		while (rs.next()) {
			...
			if (null != sb.getTCOMPANY_SEQ_ID()
					&& !sb.getTCOMPANY_SEQ_ID().equals("")) {
	...
	private boolean isSavedInStockHis(StockBO sb, Connection conn_h)
		...
		String SQL = "select 1 from TRANS_COMPANY_STOCK_HIS_TEMP where TCOMPANY_SEQ_ID = ? and PRODUCT_SEQ_ID = ? and YEARMONTH = ?";
	...
	private void updateStockHIS(StockBO sb, Connection conn_h)
			throws SQLException {
		String SQL = "update TRANS_COMPANY_STOCK_HIS_TEMP set QUANTITY = ?,PROJECT_SID = '10|12',CREATE_DATE=sysdate where TCOMPANY_SEQ_ID = ? and PRODUCT_SEQ_ID = ? and YEARMONTH = ? ";
		PreparedStatement pstmt = conn_h.prepareStatement(SQL);
	...
	private void insertStockHIS(StockBO sb, Connection conn_h)
			throws SQLException {
		String SQL = "insert into TRANS_COMPANY_STOCK_HIS_TEMP values(?,?,?,?,'10|12',sysdate,'mis') ";
	...
	private boolean putInTbl(StockBO sbo_shipment, Connection conn_h,
		...
		// 1.判断该笔记录是否已存在
		boolean isSaved = isSavedInShipment(sbo_shipment, conn_h);
		boolean flag = false;
		if (isSaved) {
	...
	private boolean updateAfter25Shipment(StockBO sbo_shipment,
		...
		String sql = "update TRANS_SAP_AFTER25SHIPMENT_TEMP set QTY=? ,UPDATE_DATE=sysdate  where CUSTOMER_ID = ? and MATERIAL_ID = ? ";
		PreparedStatement pstmt = conn_h.prepareStatement(sql);
	...
	private boolean insertAfter25Shipment(StockBO sbo_shipment,
			Connection conn_h) throws SQLException {
		String sql = "insert into TRANS_SAP_AFTER25SHIPMENT_TEMP (CUSTOMER_ID,MATERIAL_ID,QTY,UPDATE_DATE) values(?,?,?,sysdate) ";
	 ...
	 * @throws SQLException
	 */
	private boolean isSavedInShipment(StockBO sb, Connection conn_h)
			throws SQLException {
		String SQL = "select 1 from TRANS_SAP_AFTER25SHIPMENT_TEMP where CUSTOMER_ID = ? and MATERIAL_ID = ? ";
		PreparedStatement pstmt = conn_h.prepareStatement(SQL);
	...
	private boolean getCustomerStorageByDate(Connection conn_h)
		...
		boolean flag = false;
		StringBuffer SQL = new StringBuffer();
		String del_sql = "delete from TRANS_CUSTOMER_STORAGE where yearmonth = '"
				+ LAST_YEARMONTH + "' and DAY = '" + STOCK_DATE + "' ";
		SQL.append("insert into TRANS_CUSTOMER_STORAGE(");
		...
		SQL.append("where  c.yearmonth = '" + LAST_YEARMONTH
				+ "' and c.DAY = '" + STOCK_DATE
				+ "' and length(c.PROD_ID)=18)");
	...
	private boolean isSaved(StockBO sb, Connection conn_h) throws SQLException {
		boolean isSaved = false;
		String SQL = "select 1 from TRANS_CUSTOMER_STOCK_MERGE where TCOMPANY_SEQ_ID = ? and PRODUCT_SEQ_ID = ? and YEARMONTH = ?";

		PreparedStatement pstmt = conn_h.prepareStatement(SQL);
		if (sb.getTCOMPANY_SEQ_ID() == null
				|| sb.getTCOMPANY_SEQ_ID().equals("")) {
			logger.info("sb.forwarder_id:" + sb.getCUSTOMER_ID() + "("
					+ sb.getTCOMPANY_SEQ_ID() + ")");
		}
		pstmt.setInt(1, Integer.parseInt(sb.getTCOMPANY_SEQ_ID()));
		pstmt.setString(2, sb.getPRODUCT_SEQ_ID());
		pstmt.setString(3, sb.getYEARMONTH());
	...
	private void insertStockMerge(StockBO sb, Connection conn_h)
			throws SQLException {
		String SQL = "insert into TRANS_CUSTOMER_STOCK_MERGE values(?,?,?,?,'1',sysdate,'mis') ";
		PreparedStatement pstmt = conn_h.prepareStatement(SQL);
		pstmt.setInt(1, Integer.parseInt(sb.getTCOMPANY_SEQ_ID()));
		pstmt.setString(2, sb.getPRODUCT_SEQ_ID());
		pstmt.setBigDecimal(3, sb.getQTY());
	...
	private void updateStockMerge(StockBO sb, Connection conn_h)
			throws SQLException {
		String SQL = "update TRANS_CUSTOMER_STOCK_MERGE set QUANTITY = QUANTITY+? where TCOMPANY_SEQ_ID = ? and PRODUCT_SEQ_ID = ? and YEARMONTH = ? ";
	...
	private boolean flushAfter25Shipment(Connection conn_h) throws SQLException {
		String sql = "delete TRANS_SAP_AFTER25SHIPMENT_TEMP  ";
		PreparedStatement pstmt = conn_h.prepareStatement(sql);
		System.out
				.println("     --- 正在清空经销商网站录入库存表 - TRANS_SAP_AFTER25SHIPMENT_TEMP --- ");
		pstmt.execute();
		pstmt.close();
		return true;
	}
	 ...
	 * @throws SQLException 
	 */
	private String getFWD_SID_By_ID(String merge_id, Connection conn_h) throws SQLException {
